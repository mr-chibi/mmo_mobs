...
# Spawn Mob:
$execute if entity @e[type=minecraft:$(type),distance=0..10,tag=!mmo_engine_mob] as @e[type=minecraft:$(type),distance=0..10,sort=nearest,tag=!mmo_engine_mob] at @s run function mmo_mob_engine:random/default/spawning with storage minecraft:mmo_mob_id database[0]
...
#
$execute if entity @e[type=minecraft:$(type),distance=0..10,tag=mmo_engine_mob] run tag @s remove mmo_mob_spawning