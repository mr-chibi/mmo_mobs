...
$execute if entity @e[type=minecraft:$(type),distance=0..5] run data modify storage minecraft:mmo_custom_mobs database.$(type) prepend from storage minecraft:mmo_custom_mobs database.$(type)[0]
...
# Reset:
$execute if entity @e[type=minecraft:$(type),distance=0..5] run tag @s remove prev_custom_mob